from langchain.llms.base import LLM
from typing import Any, List, Mapping, Optional
import requests

class CustomLLM(LLM):
    """Custom LLM class that interacts with a local FastAPI server."""

    # The URL of the FastAPI endpoint
    endpoint: str = "http://127.0.0.1:8000/generate/"

    @property
    def _llm_type(self) -> str:
        return "custom"

    def _call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
        """
        Make a POST request to the FastAPI endpoint and return the generated text.
        """
        # The FastAPI endpoint expects a JSON payload with a "text" key
        payload = {"text": prompt}

        try:
            response = requests.post(self.endpoint, json=payload)
            response.raise_for_status()  # Raise an exception for bad status codes

            # The FastAPI endpoint returns a JSON object with a "generated_text" key
            result = response.json()
            return result.get("generated_text", "")

        except requests.exceptions.RequestException as e:
            return f"Error from API: {e}"

    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        """Get the identifying parameters."""
        return {"endpoint": self.endpoint}

class CustomSummarizer(LLM):
    """Custom LLM class for summarization that interacts with a local FastAPI server."""

    endpoint: str = "http://127.0.0.1:8000/summarize/"

    @property
    def _llm_type(self) -> str:
        return "custom_summarizer"

    def _call(self, prompt: str, stop: Optional[List[str]] = None) -> str:
        payload = {"text": prompt}
        try:
            response = requests.post(self.endpoint, json=payload)
            response.raise_for_status()
            result = response.json()
            return result.get("summary_text", "")
        except requests.exceptions.RequestException as e:
            return f"Error from API: {e}"

    @property
    def _identifying_params(self) -> Mapping[str, Any]:
        return {"endpoint": self.endpoint}

# --- Example Usage ---
if __name__ == "__main__":
    # 1. Make sure your FastAPI server is running in a separate terminal:
    #    uvicorn main:app --host 0.0.0.0 --port 8000

    print("--- Testing Text Generation ---")
    # 2. Instantiate the custom LLM for generation
    llm_generator = CustomLLM(endpoint="http://127.0.0.1:8000/generate/")

    # 3. Send a prompt to the LLM for generation
    prompt_gen = "In a world where AI rules,"
    response_gen = llm_generator.invoke(prompt_gen)

    print(f"Prompt (Generation): {prompt_gen}")
    print(f"Response (Generation): {response_gen}")

    print("--- Testing Text Summarization ---")
    # 4. Instantiate the custom LLM for summarization
    llm_summarizer = CustomSummarizer(endpoint="http://127.0.0.1:8000/summarize/")

    # 5. Send a prompt to the LLM for summarization
    prompt_sum = """The quick brown fox jumps over the lazy dog. This is a classic pangram, a sentence that contains every letter of the alphabet at least once. Pangrams are often used to display typefaces or to test typing skills. They are also a fun linguistic exercise. Another famous pangram is "Pack my box with five dozen liquor jugs."""
    response_sum = llm_summarizer.invoke(prompt_sum)

    print(f"Prompt (Summarization): {prompt_sum}")
    print(f"Response (Summarization): {response_sum}")
